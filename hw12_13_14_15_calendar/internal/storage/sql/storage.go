...
func (s *Storage) Add(e storage.Event) (int64, error) {
	...
	startedAt, err := time.Parse(ltime.DateTimeFormat, e.DateStart)
	if err != nil {
		return 0, err
	}
	endedAt, err := time.Parse(ltime.DateTimeFormat, e.DateEnd)
	if err != nil {
		return 0, err
	}
...
func (s *Storage) ListForDate(date string) ([]storage.Event, error) {
	d, err := time.Parse(ltime.DateFormat, date)
	if err != nil {
		return nil, err
	}
	start, end := ltime.DayStartAndEnd(d)
	return s.getEventsInDateRange(start, end)
}

func (s *Storage) ListForWeek(date string) ([]storage.Event, error) {
	d, err := time.Parse(ltime.DateFormat, date)
	if err != nil {
		return nil, err
	}
	start, end := ltime.WeekStartAndEnd(d)
	return s.getEventsInDateRange(start, end)
}

func (s *Storage) ListForMonth(date string) ([]storage.Event, error) {
	d, err := time.Parse(ltime.DateFormat, date)
	if err != nil {
		return nil, err
	}
	start, end := ltime.MonthStartAndEnd(d)
	return s.getEventsInDateRange(start, end)
}
